function mutateOrder(individual) {
  ...
  let newIdentity = individual.substr(0, initialPosition)
    + individual[targetPosition]
    + individual.substr(initialPosition + 1);
  newIdentity = newIdentity.substr(0, targetPosition)
    + individual[initialPosition]
    + newIdentity.substr(targetPosition + 1);
  return newIdentity;
}

function createMutation(sample) {
  return (individual) => {
    ...
    return individual.substr(0, i) + filteredSample[k] + individual.substr(i + 1);
  };
}
